...
wfs = load(ff);
nC = 102;
wfs = waveforms.v2t(wfs.wfs, nC);
T = wfs;

[ali tau mMPmask] = waveforms.tAlignOnUpsampleMean(T, 'restrictToNMaximalValues', 10, 'initAlignment', []);
% [ali2 tau2 mMPmask] = waveforms.tAlignOnUpsampleMean(ali, 'maxIdx', 5);


%%
X = waveforms.v4plot(waveforms.t2v(T(:,1:2:end,1:4:end)), size(T,2)/2)';
Y = waveforms.v4plot(waveforms.t2v(ali(:,1:2:end,1:4:end)), size(T,2)/2)';
...
% xvsf = mysort.util.calculateXIvsF(waveforms.t2v(T),waveforms.t2v(T),1,1);
