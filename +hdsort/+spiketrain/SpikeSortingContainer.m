    ...
    methods
        ...
        function self = SpikeSortingContainer(name, gdf, varargin)
            ...
            if ~isempty(P.templateWfs)
                assert(~isempty(P.templateCutLeft), 'If template hdsort.waveforms.are given, cutleft has to be set!');
                if ~isempty(P.templateCutLength)
                    assert(P.templateCutLength == size(self.templateWfs,1), 'TemplateCutLength does not match given hdsort.waveforms.');
                end
            end
