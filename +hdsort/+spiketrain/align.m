...
function R = align(St1, St2, maxJitter, maxShift, maxOverlapDist, St1IDs, St2IDs, varargin)
...
% assignments use:
%
%     R = hdsort.spiketrain.alignGDFs(gdf1, gdf2, 10, 10, 10);
%     hdsort.plot.printEvaluationTable(R, 'ignoreEmptySorted', 0);
%     hdsort.plot.alignment(R)
%
% Inputs:
...
%         the final results
%
P.minimizeError = false;
P.maximizeTPR = false;
P.weightingMatrix = [];
P.useparfor = true;
P = hdsort.util.parseInputs(P, varargin, 'error');

assert(P.minimizeError + P.maximizeTPR < 2, 'Choose either one or the other!')

P.maxJitter = maxJitter;
P.maxShift = maxShift;
P.maxOverlapDist = maxOverlapDist;
...
% spike trains).
 disp('Start hdsort.spiketrain.compare...')
R.RsingleComp = hdsort.spiketrain.compare(St1, St2, maxJitter, maxShift, P.useparfor);
...
if maxShift > 0
    ...
    R = hdsort.spiketrain.align(St1, St2, maxJitter, 0, maxOverlapDist, R.St1IDs, R.St2IDs, ...
        'weightingMatrix', P.weightingMatrix, 'useparfor', P.useparfor);
    return
end

% Check every ground truth spike, whether there is another one nearby
% as an overlap.
[R.O R.nO] = hdsort.spiketrain.checkForOverlaps(St1, maxOverlapDist);
...
%----------------------------------------------------------------------
    function R = computeSpikeTrainAssignment(R, St1, St2)
        ...
        % Not true positive are maximized, but the error minimized:
        if P.minimizeError
            TP = R.RsingleComp.TP - R.RsingleComp.FN - R.RsingleComp.FP;
        end
        
        if ~isempty(P.weightingMatrix)
            TP = TP .* P.weightingMatrix;
            R.weightingMatrix = P.weightingMatrix;
        end
        
        if P.maximizeTPR
            TP = R.RsingleComp.TP ./ repmat(R.RsingleComp.N2', size(R.RsingleComp.TP, 1), 1);
        end
        
        
        R.assignmentMatrix = TP;
        ...
        while count < R.nSt1*R.nSt2
            ...
            [i,j,v] = hdsort.util.matrixArgMax(TP);
