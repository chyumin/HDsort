function ifo = findSubH5info(h5info, h5path)
    ...
    if ischar(h5path)
        h5path = hdsort.filewrapper.hdf5.splitH5path(h5path);
    end
    
    % check if we are the root
    if isfield(h5info, 'GroupHierarchy')
        ifo = hdsort.filewrapper.hdf5.findSubH5info(h5info.GroupHierarchy, h5path);
        return
    end
    ...
    if ~isempty(idx) && (idx+length(['/' h5path{1}])-1 == length(h5info.Name))
        ...
        ifo = hdsort.filewrapper.hdf5.findSubH5info(h5info, h5path(2:end));
        if ~isempty(ifo)
            return
        end
    end
    ...
    if isfield(h5info, 'Groups')
        for i=1:length(h5info.Groups)
            ifo = hdsort.filewrapper.hdf5.findSubH5info(h5info.Groups(i), h5path);
            if ~isempty(ifo)
                return
            end
        end
    ...
    if isfield(h5info, 'Datasets')
        for i=1:length(h5info.Datasets)
            ifo = hdsort.filewrapper.hdf5.findSubH5info(h5info.Datasets(i), h5path);
            if ~isempty(ifo)
                return
            end
        end        
