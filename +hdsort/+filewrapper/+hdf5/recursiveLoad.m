...
function D = recursiveLoad(fname, hinfo)
    if nargin == 1
        ...
        for i=1:length(hinfo.GroupHierarchy)
            D{i} = hdsort.filewrapper.hdf5.recursiveLoad(fname, hinfo.GroupHierarchy(i));
        end
    elseif ischar(hinfo)
        str = hinfo;
        hinfo = hdf5info(fname);
        ...
        D = hdsort.filewrapper.hdf5.recursiveLoad(fname, hinfo);
    else
        isVar = true;
        ...
        if isfield(hinfo, 'Groups')
            ...
            for i=1:length(hinfo.Groups)
                ...
                TMP = hdsort.filewrapper.hdf5.recursiveLoad(fname, hinfo.Groups(i));
