...
%             self.chans = self.chans(:)';
        ...
        function initFiles(self)
            ...
            self.handles.proc = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, '/bFileIsInProcess', [1 1], [1 1], 'H5T_NATIVE_INT');
            self.handles.proc(1,1) = int32(1);     
            
            if ~self.P.save_as_binary
                self.handles.sig = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/sig'], ...
                ...
                self.handles.sig = hdsort.filewrapper.util.BinaryFileMatrix(self.outFileNameBinary, [1, self.h5infos.maxDims(2)], 'writable', true);
                
                % Save a link to the binary file into /sig:
                [path, name, ext] = fileparts(self.outFileNameBinary);
                fileNameBinary = [name ext];
                
                sig_link = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/sig'], [1 length(fileNameBinary) ], [1 length(fileNameBinary) ], 'H5T_C_S1');
                sig_link(1,1:length(fileNameBinary)) = fileNameBinary;
                clear sig_link;
                bin_dims = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/bin_dims'], [1 2], [1 2], 'H5T_NATIVE_LONG');
                bin_dims(1, :) = int32(self.h5infos.maxDims);
                clear bin_dims;                
            end
            
            % SR
            sr_ = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/sr'], [1 1], [1 1], 'H5T_NATIVE_INT');
            sr_(1,1) = int32(self.srate);
            clear sr_    
            
            % VERSION
            version = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/version'], [1 1], [1 1], 'H5T_NATIVE_INT');
            version(1,1) = int32(self.P.fileVersion);
            clear version
            
            % GAIN
            gain = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/gain'], [1 4], [1 4], 'H5T_NATIVE_DOUBLE');
            gain(1,2:4) = self.P.amplifierGains;
            gain(1,1) = prod(self.P.amplifierGains); % total gain
            clear gain

            % ADC range and resolution
            gain = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/adc_resolution'], [1 1], [1 1], 'H5T_NATIVE_DOUBLE');
            gain(1,1) = self.P.adc_resolution;
            gain = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/adc_range'], [1 1], [1 1], 'H5T_NATIVE_DOUBLE');
        ...
        function saveFramenumbers(self, firstFN, lastFN, missingIDX, missingLength)
            ...
            ffn = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/frame_numbers/first_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ffn(1,1) = int32(firstFN);
            clear ffn
            ffn = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/frame_numbers/last_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ffn(1,1) = int32(lastFN);
            clear ffn
            ffn = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/frame_numbers/dataDims'], [1 2], [1 2], 'H5T_NATIVE_INT');
            ffn(1,:) = int32(self.h5infos.maxDims);
            clear ffn
            if ~isempty(missingIDX)
                ffn = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/frame_numbers/missing_fns'], [L 2], [L 2], 'H5T_NATIVE_INT');
                ffn(:,:) = [missingIDX(:) missingLength(:)];
            else
                ffn = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/frame_numbers/missing_fns'], [1 1], [1 1], 'H5T_NATIVE_INT');
                ffn(1,1) = int32(-1);
            end
            clear ffn
        end
        ...
        function saveRawH5File(self, ffile)
            ord = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/source_files/raw_h5'], [1 length(ffile)], [1 length(ffile)], 'H5T_C_S1');
        ...
        function saveselfpingFile(self, mfile)
            ord = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/source_files/selfping_file'], [1 length(mfile)], [1 length(mfile)], 'H5T_C_S1');
        ...
        function saveChipID(self, chipid)
            % CHIP ID
            h = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/chipid'], [1 1], [1 1], 'H5T_NATIVE_INT');
            h(1,1) = int32(chipid);
            clear h   
        end        
        ...
        function saveNewChannelList(self)
            ...
            x = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/channel_nr'], [1 nC], [1 nC], 'H5T_NATIVE_DOUBLE');
            x(1,:) = self.elNo;
            clear x
            x = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/channel_posx'], [1 nC], [1 nC], 'H5T_NATIVE_DOUBLE');
            x(1,:) = self.elX;
            clear x
            x = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/channel_posy'], [1 nC], [1 nC], 'H5T_NATIVE_DOUBLE');
            x(1,:) = self.elY;
            clear x
            x = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.outFileNameH5, [self.P.sessionName '/channel_connected'], [1 nC], [1 nC], 'H5T_NATIVE_DOUBLE');
            x(1,:) = ones(1, nC);
            clear x
        end
    end
