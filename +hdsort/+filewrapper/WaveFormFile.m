classdef WaveFormFile < hdsort.filewrapper.FileWrapperInterface
    ...
    methods
        ...
        function self = WaveFormFile(fileName, varargin) % cutLeft, Tf, nC, samplesPerSecond
            ...
            self = self@hdsort.filewrapper.FileWrapperInterface('WaveFormFile', P.samplesPerSecond, P.MultiElectrode);
            ...
            if exist(self.fileName) ~= 2
                ...
                self.wfs = hdsort.filewrapper.util.BinaryFileMatrix(self.binFile, [1 self.nC*self.Tf], ...
                    'writable', true, 'precision', P.precision);
                
                self.gdf = hdsort.filewrapper.hdf5.matrixCreate(self.fileName, self.datasets.gdf, ...
                    [0 self.nRowsGdf], [-1, self.nRowsGdf], 'H5T_NATIVE_DOUBLE', [1, 1], 0);
                
                self.binDims = hdsort.filewrapper.hdf5.matrixCreate(self.fileName, self.datasets.binDims, ...
                ...
                binFileName = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.fileName, self.datasets.binFile, [1 length(self.binFileName) ], [1 length(self.binFileName) ], 'H5T_C_S1');
                binFileName(1,1:length(self.binFileName)) = self.binFileName;
                clear binFileName;
                
                precision_ = hdsort.filewrapper.hdf5.createVariableAndOrFile(self.fileName, self.datasets.precision, [1 length(self.precision) ], [1 length(self.precision) ], 'H5T_C_S1');
                ...
                nC_ = hdsort.filewrapper.hdf5.matrixCreate(self.fileName, self.datasets.nC, ...
                    dims, maxDims, h5type, chunkDims, deflation);
                nC_(1,1) = int32(self.nC);
                clear nC_
                
                cutLeft_ = hdsort.filewrapper.hdf5.matrixCreate(self.fileName, self.datasets.cutLeft, ...
                    dims, maxDims, h5type, chunkDims, deflation);
                cutLeft_(1,1) = int32(self.cutLeft);
                clear cutLeft_
                
                samplesPerSecond_ = hdsort.filewrapper.hdf5.matrixCreate(self.fileName, self.datasets.samplesPerSecond, ...
                    dims, maxDims, h5type, chunkDims, deflation);
                ...
                self.gdf = hdsort.filewrapper.hdf5.matrix(self.fileName, self.datasets.gdf, ~self.writable);
                self.binDims = hdsort.filewrapper.hdf5.matrix(self.fileName, self.datasets.binDims, ~self.writable);
                ...
                if binDims(1)
                    self.wfs = hdsort.filewrapper.util.BinaryFileMatrix(self.binFile, binDims,...
