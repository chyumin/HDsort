...
smad = hdsort.util.mad(X(noiseIdx,:)')';
smad(smad<=10*eps) = 1000;
if nargout > 1
    commonNoiseEpochs = hdsort.epoch.fromBinaryVector(noiseIdx);
    notNoiseEpochsSet = hdsort.epoch.removeShort(hdsort.epoch.flip(commonNoiseEpochs, L), Tf);
end
