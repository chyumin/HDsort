...
warning('This function is depricated! Use mysort.wf.* instead!');
    P.nIter = 3;
    P = mysort.hdsort.util.parseInputs(P, 'alignWaveformsTaylorExpansionMean', varargin);
    ...
    for iter=1:P.nIter
        ...
        S   = mysort.hdsort.util.shiftRowsInterpolated(S, tau, nC);
    ...
    function [tau last_tau] = alignStep(X, nC, last_tau)
        mx = mysort.hdsort.util.v2m(mean(X,1), nC);
        range = 1:size(mx,2);
        tau = zeros(size(X,1), 1);
        for i=1:size(X,1)
            if abs(last_tau(i))>.3
                xsinc = mysort.hdsort.util.sincfun(mysort.hdsort.util.v2m(X(i,:), nC));            
