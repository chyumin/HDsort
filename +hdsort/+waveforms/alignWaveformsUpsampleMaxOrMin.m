...
warning('This function is depricated! Use mysort.wf.* instead!');
    ...
    P.UP = 3;
    P.downsample = 1;
    P = mysort.hdsort.util.parseInputs(P, 'alignWaveformsUpsampleMaxOrMin', varargin);

    Sup = mysort.hdsort.util.resampleTensor(mysort.hdsort.util.m2t(S, nC), P.UP,1);
    Sup = mysort.hdsort.util.t2m(Sup);
    [tau Sup] = mysort.hdsort.util.alignWaveformsOnMaxOrMin(Sup, nC, 'maxIdx', P.UP*P.maxIdx);
    if P.downsample
        tau = tau/P.UP;
