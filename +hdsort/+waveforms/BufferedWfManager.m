classdef BufferedWfManager < hdsort.waveforms.WfManagerWithDataSource
    ...
    methods
        ...
        function self = BufferedWfManager(wfDataSource, eventTimes, eventChans, eventIDs, cutleft, cutlength)
            self = self@hdsort.waveforms.WfManagerWithDataSource(wfDataSource, eventTimes, eventChans, eventIDs);
...
%             assert(any(self.MultiElectrode.getElIdx4ElNumber(electrodeNumber)==-1), 'could not find requested electrode number!')
            ...
            if ~isempty(notBufferedIdx)
                ...
                wfs = getWaveform4Idx@hdsort.waveforms.WfManagerWithDataSource(self,...
                        wfIdx, self.wfbufferCutLeft, self.wfbufferCutLength);
                %wfs = getWaveform4Idx@hdsort.waveforms.WfManagerWithDataSource(self,wfIdx, self.wfbufferCutLeft, self.wfbufferCutLength);
            ...
            if nargin < 5 || isempty(electrodeNumber)
                ...
                chansubidx = hdsort.waveforms.vSubChannelIdx(self.wfbufferCutLength, self.MultiElectrode.getNElectrodes(), channelidx);
                wfs = self.wfbuffer(self.wfbufferIdx(idx,1), chansubidx);
            end
        end   
        %------------------------------------------------------------------
        function wfManager = getWfManger4SubIdx(self, idx)
            wfManager = hdsort.waveforms.BufferedWfManager(self.wfDataSource,...
