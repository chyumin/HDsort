...
if ~isempty(P.wfsFile)
    ...
    if exist(P.wfsFile, 'file') ~= 2
        ...
        cutLeft = P.cutLeft;
        Tf = P.Tf;
        WFS = hdsort.file.WaveFormFile(P.wfsFile, 'cutLeft', cutLeft, ...
        ...
        while chunker.hasNextChunk()
            ...
            WFS.addWaveforms(wfs_, chunked_gdf);
        end
    else
        %% Load precut waveforms:
        WFS = hdsort.waveforms.WaveFormFile(P.wfsFile);
        gdf_reduced = WFS.getGdf();
        
        Tf = WFS.getTf();
        nC = WFS.getNChannels();
        cutLeft = WFS.getCutLeft();
    ...
    tic
    [templates, nSpikesPerTemplate] = estimateTemplates(gdf_reduced, Tf, nC);
    toc
    ...
    if size(gdf_reduced, 1) < maxBytes / (8*Tf*nC)
        ...
        if ~isempty(gdf_reduced)
            ...
            t = tic;
            WFS = DS.getWaveform(gdf_reduced(:,2), cutLeft, Tf);
            t = toc(t);
        ...
        while chunker_over_units.hasNextChunk()
            ...
            if ~isempty(gdf_reduced_over_units)
                fprintf('Cutting Spikes on all channels to estimate full templates...\n');
                t = tic;
                WFS = DS.getWaveform(gdf_reduced_over_units(:,2), cutLeft, Tf);
                t = toc(t);
                fprintf('Cutting Spikes done. (%.2fs)\n', t);
            else
                WFS = [];
            ...
            % #################### Actually estimate templates ####################
            tic
            [templates_, nSpikesPerTemplate_] = estimateTemplates(gdf_reduced_over_units, Tf, nC);
            toc
            ...
            clear WFS templates_ nSpikesPerTemplate_
        end
...
% ---------------------------------------------------------------------
    function [templates, nSpikesPerTemplate] = estimateTemplates(gdf_reduced, Tf, nC)
        ...
        for ii = 1:nUnits
            ...
            templates(:,:,ii) = hdsort.waveforms.v2t( median(WFS(idx, :),1), nC);
