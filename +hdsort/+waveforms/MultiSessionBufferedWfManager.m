classdef MultiSessionBufferedWfManager < hdsort.waveforms.WfManagerWithDataSource
    ...
    methods
        ...
        function self = MultiSessionBufferedWfManager(wfDataSource, eventTimes, eventChans, eventSessions, cutleft, cutlength, eventIDs, ME)
            ...
            self = self@hdsort.waveforms.WfManagerWithDataSource(wfDataSource, eventTimes, eventChans, eventIDs, ME);
            self.wfSessionIdx = uniqSess;
            self.eventSessions = eventSessions;
            self.wfbufferCutLeft = cutleft;
            self.wfbufferCutLength = cutlength;
            self.wfSingleSessionWfBuffers = hdsort.waveforms.BufferedWfManager.empty();
%             assert(~any(eventChans==0), 'Event channels must be greater zero!');
            for i=1:length(self.wfSessionIdx)
                ...
                self.wfSingleSessionWfBuffers(i) = hdsort.waveforms.BufferedWfManager(...
        ...
        function [T, ME, wfsAllSessions, nWfsPerElectrode] = getTemplate4Idx(self, idx, varargin)
            ...
            for i=1:length(sessionIdx)
                ...
                if ~isempty(my_elidx)
                    ...
                    T(my_elidx, :) = T(my_elidx, :) + hdsort.waveforms.v2m(median(wfs, 1), length(my_elidx))*nWfs(i);                
                end
            end
            for i=1:nElectrodes
...
%             assert(all(~isnan(T(:))), 'nan in template calculation!');
        ...
        function wfManager = getWfManger4SubIdx(self, idx)
            wfManager = hdsort.waveforms.MultiSessionChildWfManager(self, idx);
%             wfManager = hdsort.waveforms.MultiSessionBufferedWfManager(self.wfDataSource,...
