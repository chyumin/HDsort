...
warning('This function is depricated! Use mysort.wf.* instead!');
    ...
    X = X + randn(size(X));
    mysort.hdsort.plot.spikes(X,'nC',nC);
    title('raw traces with hdsort.noise.);
...
%     title('Aligned Spikes');
    ...
    [tau Y] = mysort.hdsort.util.alignWaveformsInterpolatedMean(X, 2);
    fprintf('Shift Error after Alignment %.4f: \n', sum(abs(-tau-stau)));
    mysort.hdsort.plot.spikes(Y,'nC',nC);
    title('Aligned Traces Mean');    
   
    [tau_min Y] = mysort.hdsort.util.alignWaveformsInterpolatedMin(X, 2);
