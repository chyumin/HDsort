function [groups maxT D] = mergeTemplates(T, meanNoiseStd, varargin)
    P.maxRelativeDistance = .35; % in hdsort.noise.std
    P.minCorrelation = .90; % fraction
    P.maxMeanDist = []; %.3; % in hdsort.noise.std
    ...
    while bMerged
        ...
        if ~any(A(:))
            ...
            MD(~A) = 1;
            [reduced_a reduced_b] = mysort.hdsort.util.matrixArgMax(-MD);
...
%     meanDistCriterion = DISTS < .3;   % in hdsort.noise.std
...
%% Or use Clustering    
%         nDim = 14;
%         fetX = mysort.hdsort.util.dimReductionPCA(M', nDim, [], 3*1000000);
...
%         uclasses = unique(ids);
%         clustering.templates = mysort.hdsort.util.calculateClassMeans(VT, ids, 'usemedian');
%         mysort.hdsort.plot.clustering(fetX, ids);
%         groupIdx = {};
%         groups = {};
