...
Q = [];
for i=1:maxShift
    ipf = hdsort.util.interpolfun(Y(i,:));
...
R = [];
for i=1:maxShift
    sf = hdsort.waveforms.mSincfun(Y(i,:));
...
% min_y = min_x_i;
% for i=1:size(Y,1)
%     [min_x_i(i) min_y_i(i) min_x(i) min_y(i)] = hdsort.util.findMinInterp(-Y(i,:));
% end
% min_x_i = min_x_i-median(min_x_i);
% % [tau2 Ya2] = hdsort.util.alignWaveformsInterpolatedMean(Y, 1);
% % subplot(3,1,3)
% % plot(Ya2');
