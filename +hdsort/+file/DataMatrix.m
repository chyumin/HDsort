classdef DataMatrix < hdsort.file.FileWrapperInterface
    ...
    methods
        ...
        function self = DataMatrix(M, varargin)
            ...
            self = self@hdsort.file.FileWrapperInterface('DataMatrix', P.samplesPerSecond, P.MultiElectrode)
            
            if isempty(self.MultiElectrode)
                N = size(M, 2);
                self.MultiElectrode = hdsort.file.MultiElectrode([ones(1, N); 1:N]', [1:N]);
            end
