classdef MultiElectrode < handle
    ...
    methods
        ...
        function save2File(self, fname, h5path)
            S = self.toStruct();
            hdsort.file.hdf5.recursiveSave(fname, S, h5path);
        end
        %------------------------------------------------------------------
        function cp = copy(self)
            S = self.toStruct();
            cp = hdsort.file.MultiElectrode(S);
        end
        ...
        function [merged mergedElIdx] = merge(self, ME)
            ...
            merged = self.copy();
            assert(isa(ME, 'hdsort.file.MultiElectrode'), 'ME must be a multi electrode!');
        ...
        function ME = getSubElectrode4ElIdx(self, elidx)
            ...
            ME = hdsort.file.MultiElectrode(epos, enum, ...
                'dataSource', [], ...
