...
%         disp('File already exist, trying to create h5 variable...')
        ...
        disp('Creating File...')
        fileID = hdsort.file.hdf5.createFile(fname);
    end
    dataspaceID = hdsort.file.hdf5.createDataspace(dims, maxDims);
    
    if isempty(chunkDims)
        datasetID = hdsort.file.hdf5.createDataset(fileID, ...
            dataspaceID, h5path, h5type);
    else
        datasetID = hdsort.file.hdf5.createDatasetChunked(fileID, ...
            dataspaceID, h5path, h5type, ...
            chunkDims, deflation);
    end
    H5D.close(datasetID);
    H5F.close(fileID); 
    M = hdsort.file.hdf5.matrix(fname, h5path, bReadOnly);