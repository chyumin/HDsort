        ...
        if length(val) > 1
            ...
            for k=1:length(val)
                ...
                for i=1:length(flist)
                    hdsort.file.hdf5.recursiveSave(fname, val(k).(flist{i}), [tree '/' myname flist{i}], varMode);
                end
            end
        else
            for i=1:length(flist)
                hdsort.file.hdf5.recursiveSave(fname, val.(flist{i}), [tree '/' flist{i}], varMode);
            end
        end
    else
        if iscell(val)
            hdsort.file.hdf5writeCell(fname, tree, val, varMode);
