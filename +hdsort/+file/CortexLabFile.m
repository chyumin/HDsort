classdef CortexLabFile < hdsort.file.SingleFileWrapper
    ...
    methods
        ...
        function self = CortexLabFile(fileName, prmFile, varargin)
            ...
            %% Read Parameter file:
            prm = hdsort.file.CortexLabFile.readPRM(prmFile);
            ...
            if 0
                ...
                multiElectrode_ = hdsort.file.MultiElectrode([xPos(sortidx) yPos(sortidx)], elns(sortidx));
            else
            ...
            self = self@hdsort.file.SingleFileWrapper('CortexLabFile', samplingRate_, multiElectrode_, fileName, varargin{:});
...
%                     missingFrames = hdsort.file.util.getMissingFrameNumbers(self.getFrameNumbers());
%                 end
%                 if nargin == 2
...
%         end
        ...
        function [outFile, P] = preprocessFile(self, path, varargin)
            ...
            proc = hdsort.file.hdf5.createVariableAndOrFile(outFile, '/bFileIsInProcess', [1 1], [1 1], 'H5T_NATIVE_INT');
            proc(1,1) = int32(1);
            ...
            if ~P.save_as_binary
                sig = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/sig'], ...
                ...
                sig = hdsort.file.util.BinaryFileMatrix(P.binFile, [1 nC_effective], 'writable', true);
                
                % Save a link to the binary file into /sig:
                binFileName = [name, '.dat'];
                sig_link = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/sig'], [1 length(binFileName) ], [1 length(binFileName) ], 'H5T_C_S1');
                sig_link(1,1:length(binFileName)) = binFileName;
                clear sig_link;
                
                bin_dims = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/bin_dims'], [1 2], [1 2], 'H5T_NATIVE_LONG');
            ...
            if 0
                ...
                missingFrames = hdsort.file.util.getMissingFrameNumbers(FRAMES);
                ffn = [missingFrames.begin(:) missingFrames.length(:)];
                FRAMES_RESTORED = hdsort.file.util.getFrameNumbersFromMissing(missingFrames.first, missingFrames.last, ffn')
            end
            
            ffn = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/frame_numbers/first_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ffn(1,1) = int32(missingFrames.first);
            clear ffn
            ffn = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/frame_numbers/last_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ffn(1,1) = int32(missingFrames.last);
            clear ffn
            ffn = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/frame_numbers/dataDims'], [1 2], [1 2], 'H5T_NATIVE_INT');
            ffn(1,:) = int32(self.dims);
            clear ffn
            if missingFrames.n > 0
                lL = numel(missingFrames.begin);
                ffn = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/frame_numbers/missing_fns'], [lL 2], [lL 2], 'H5T_NATIVE_INT');
                ffn(:,:) = [missingFrames.begin(:) missingFrames.length(:)];
            else
                ffn = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/frame_numbers/missing_fns'], [1 1], [1 1], 'H5T_NATIVE_INT');
                ffn(1,1) = int32(-1);
            end
            clear ffn
            ...
            function prepareFilterVariables()
                pref = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/prefiltered'], [1 1], [1 1], 'H5T_NATIVE_INT');
                pref(1,1) = int32(1);
                clear pref
                high = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/highpass'], [1 1], [1 1], 'H5T_NATIVE_INT');
                high(1,1) = int32(h);
                clear high
                low = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/lowpass'], [1 1], [1 1], 'H5T_NATIVE_INT');
                low(1,1) = int32(l);
                clear low
                down = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/downsamplefactor'], [1 1], [1 1], 'H5T_NATIVE_INT');
                down(1,1) = int32(d);
                clear down
                ord = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/order'], [1 1], [1 1], 'H5T_NATIVE_INT');
                ord(1,1) = int32(fo);
                clear type
                ord = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/type'], [1 20], [1 20], 'H5T_C_S1');
                ord(1,1:length(filterType)) = filterType;
                clear ord
                gd = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/filter/gainmultiplier'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ...
            function prepareMeaVariables()
                ...
                ord = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/source_files/raw_h5'], [1 length(self.fileName)], [1 length(self.fileName)], 'H5T_C_S1');
                ord(1,1:length(self.fileName)) = self.fileName;
                clear ord
                
                % CHIP ID
                chipid = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/chipid'], [1 1], [1 1], 'H5T_NATIVE_INT');
                chipid(1,1) = int32(fileInfo.chipid);
                clear chipid
                
                % GAIN
                gain = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/gain'], [1 4], [1 4], 'H5T_NATIVE_DOUBLE');
                gain(1,2:4) = [fileInfo.gain1 fileInfo.gain2 fileInfo.gain3];
                gain(1,1) = prod(gain(1,2:4)); % total gain
                clear gain
                
                % ADC range and resolution
                gain = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/adc_resolution'], [1 1], [1 1], 'H5T_NATIVE_DOUBLE');
                gain(1,1) = fileInfo.adc_resolution;
                gain = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/adc_range'], [1 1], [1 1], 'H5T_NATIVE_DOUBLE');
                gain(1,1) = fileInfo.adc_range;
                clear gain;
                
                % SR
                sr_ = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/sr'], [1 1], [1 1], 'H5T_NATIVE_INT');
                sr_(1,1) = int32(fileInfo.sr);
                clear sr_
                
                % VERSION
                version = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/version'], [1 1], [1 1], 'H5T_NATIVE_INT');
            ...
            function [isConnected, count] = prepareChannelList()
                ...
                cl = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/channel_list'], nC, nC, type_id);
                % get one object
                a = cl(1);
            ...
            function prepareNewChannelList(isConnected)
                ...
                idx = isConnected==1;
                idx = idx(:);
                x = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/channel_nr'], [1 nC_effective], [1 nC_effective], 'H5T_NATIVE_DOUBLE');
                x(1,1:nC_effective) = map.elNo(idx);
                clear x
                x = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/channel_posx'], [1 nC_effective], [1 nC_effective], 'H5T_NATIVE_DOUBLE');
                x(1,1:nC_effective) = map.mposx(idx);
                clear x
                x = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/channel_posy'], [1 nC_effective], [1 nC_effective], 'H5T_NATIVE_DOUBLE');
                x(1,1:nC_effective) = map.mposy(idx);
                clear x
                x = hdsort.file.hdf5.createVariableAndOrFile(outFile, [P.sessionName '/channel_connected'], [1 nC_effective], [1 nC_effective], 'H5T_NATIVE_DOUBLE');
