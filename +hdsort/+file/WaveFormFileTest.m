...
%%
delete(wfsFile);
W = hdsort.file.WaveFormFile(wfsFile, 'cutLeft', cutLeft, 'Tf', Tf, 'nC', nC);
x = W(:,:);
y = W(:,:,:);
xgdf = W.getGdf();
clear W
...
%%
W = hdsort.file.WaveFormFile(wfsFile, 'writable', false);
x = W(:,:);
y = W(:,:,:);
clear W
assert(isempty(x), '!')
...
%%
W = hdsort.file.WaveFormFile(wfsFile, 'writable', true);
x = W(:,:);
y = W(:,:,:);
assert(isempty(x), '!')
assert(isempty(y), '!')
...
%%
W = hdsort.file.WaveFormFile(wfsFile);
Wgdf = W.getGdf();
...
delete(wfsFile2);
W = hdsort.file.WaveFormFile(wfsFile2, 'cutLeft', cutLeft, 'Tf', Tf, 'nC', nC);
...
clear W
W2 = hdsort.file.WaveFormFile(wfsFile2, 'writable', false);
tW2 = W2(:,:, :);
assert(isequal(tW2, tWFS), '!')
clear W2

W3 = hdsort.file.WaveFormFile(wfsFile2, 'writable', true);
...
for ii = 1:10
    ...
    delete(wfsFile{ii});
    WF{ii} = hdsort.file.WaveFormFile(wfsFile{ii}, 'cutLeft', cutLeft, 'Tf', Tf, 'nC', nC+ii);
end
parfor ii = 1:10
...
clear WF
for ii = 1:10
    WF{ii} = hdsort.file.WaveFormFile(wfsFile{ii});
    size(WF{ii})
end
%%
clear WF
parfor ii = 1:10
    ...
    gdf = [repmat(1, 1, N), (1:N); repmat(2, 1, N), (1:N)+20]';
    wfs = 100*randn(N*2, Tf*(nC+ii));
    WF = hdsort.file.WaveFormFile(wfsFile{ii}, 'writable', true);
