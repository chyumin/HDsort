classdef BELMEAFile < hdsort.file.SingleFileWrapper
    ...
    methods
        ...
        function rawSpikes = getRawSpikes(self)
            if isempty(self.rawSpikes)
                rawSpikes_ = self.tryReadingDataset(self.dataSets.raw_spikes.name, false);
                if ~isempty(rawSpikes_)            
                    self.rawSpikes.spiketimes = double(rawSpikes_.frameno);
                    self.rawSpikes.channel = double(rawSpikes_.channel) + 1;
                    self.rawSpikes.positions = self.MultiElectrode.electrodePositions(self.rawSpikes.channel, :);
                    self.rawSpikes.electrode = self.MultiElectrode.electrodeNumbers(self.rawSpikes.channel);
                    self.rawSpikes.amplitude = double(rawSpikes_.amplitude);
                else
                    self.rawSpikes = [];
                end
            end
            rawSpikes = self.rawSpikes;
        end
