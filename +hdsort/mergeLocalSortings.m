...
%             end
            ...
            [mi1 ma1] = mysort.wf.tMinMaxPerTemplate(G(g1).templates.wfs(:,:,t1idx));
            [mi2 ma2] = mysort.wf.tMinMaxPerTemplate(G(g2).templates.wfs(:,:,t2idx));
            if 0
                figure;
            ...
            for t1 = 1:length(t1idx)
                ...
                for t2 = 1:length(t2idx)
                    ...
                    unionValidIdx = validChanIdx1 | validChanIdx2;
...
%                     end
                    ...
                    if 0
                        %%
                        figure
                        plot(G(g1).templates.wfs(:,:,t1idx(t1)))
                        %%
                        figure
                        w1 = G(g1).templates.wfs(:,:,t1idx(t1));
                        w2 = G(g2).templates.wfs(:,:,t2idx(t2));
                        plot([w1(:) w2(:)])
                        ...
                        hold on
                        plot(M1, 'g')
                        plot([1 length(M2)], [1 1]*P.percentageCutoff*absMax2, ':r')
                    end
                    ...
                    MM1 = M1(unionValidIdx);
                    MM2 = M2(unionValidIdx);
                    N = length(MM1);
                    c = 1/(N-1)*((MM1-mean(MM1))*(MM2-mean(MM2))') / sqrt(var(MM1)*var(MM2));
                    if c > P.minCorrelationForMerger | isnan(c)
